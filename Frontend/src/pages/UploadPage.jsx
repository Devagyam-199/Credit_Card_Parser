...
const UploadPage = ({ setParsedData, setLoading, loading }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-6 px-2 sm:px-4 lg:px-8">
      <div className="max-w-md w-full space-y-6 sm:space-y-8">
        ...
        <form onSubmit={handleSubmit} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop} className="space-y-4">
          <div className="flex flex-col items-center justify-center w-full h-48 sm:h-64 border-2 border-dashed rounded-lg bg-gray-50 p-4 sm:p-6">
            <div className={dragActive ? "hidden" : "text-center space-y-2"}>
            ...
            {file && (
              <p className="text-xs sm:text-sm text-gray-500 mt-2">{file.name}</p>
            )}
          </div>
          <button
            type="submit"
            disabled={!file || loading}
            className="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm sm:text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
