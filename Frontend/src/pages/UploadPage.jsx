...
const UploadPage = ({ setParsedData, setLoading, loading }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-6 px-2 sm:px-4 lg:px-8 bg-gradient-to-br from-gray-100 to-gray-200" style={{ height: 'calc(100vh - 4rem)' }}>
      <div className="max-w-md w-full space-y-6 sm:space-y-8 bg-white rounded-xl shadow-2xl p-4 sm:p-6 flex flex-col h-full overflow-hidden">
        ...
        <form onSubmit={handleSubmit} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop} className="flex-1 space-y-4 flex flex-col overflow-hidden">
          <div className="flex-1 flex flex-col items-center justify-center w-full h-full border-2 border-dashed rounded-xl bg-gray-50 p-4 transition-all duration-300 hover:border-indigo-500 hover:bg-gray-100 overflow-hidden">
            <div className={dragActive ? "hidden" : "text-center space-y-4 animate-fadeIn flex-1 flex flex-col items-center justify-center"}>
            ...
            {dragActive && (
              <div className="text-center space-y-4 animate-pulse flex-1 flex flex-col items-center justify-center">
            ...
            {file && (
              <p className="text-xs sm:text-sm text-gray-500 mt-2 bg-gray-100 px-3 py-1 rounded-full animate-slideUp">{file.name}</p>
            )}
          </div>
          <button
            type="submit"
            disabled={!file || loading}
            className="flex-shrink-0 group relative w-full flex justify-center py-3 px-6 border border-transparent text-base font-medium rounded-xl text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transform transition-all duration-300 hover:scale-105"
          >
            {loading ? (
